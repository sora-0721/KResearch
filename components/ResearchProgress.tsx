...
const TypeIcon = ({ type, persona, t }: { type: ResearchUpdate['type'], persona?: AgentPersona, t: (key: string) => string }) => {
  if (type === 'thought' && persona) {
    return getAgentInfo(persona, t).icon;
  }
  switch (type) {
    case 'thought':
...
const ResearchProgress: React.FC<ResearchProgressProps> = ({ updates, isResearching }) => {
  const { t } = useLanguage();
  const headerText = isResearching ? t('researching') : t('researchComplete');
        ...
        <div className="relative pl-8">
          ...
          {updates.map((update) => (
            ...
            <div className="absolute left-0 top-1.5 w-6 h-6 rounded-full bg-glass-light dark:bg-glass-dark border-2 border-border-light dark:border-border-dark flex items-center justify-center -translate-x-1/2">
                <TypeIcon type={update.type} persona={update.persona} t={t}/>
            </div>
            <div className="ml-2 p-3 rounded-2xl bg-glass-light/30 dark:bg-glass-dark/30 backdrop-blur-sm border border-border-light/20 dark:border-border-dark/20">
                ...
                {update.type === 'search' && Array.isArray(update.content) ? (
                    <div>
                    <span className="font-semibold capitalize text-gray-800 dark:text-gray-200">{t('searchingFor')}</span>
                    ...
                    <div>
                        <span className="font-semibold capitalize text-gray-800 dark:text-gray-200">{t('readAndSynthesized')}</span>
                        {Array.isArray(update.source) &&
                            <span className="text-xs text-gray-500 ml-1">({update.source.length} {t('sources')})</span>
                        }
                    ...
                    <div>
                        {update.persona ? (
                             <span className={`font-semibold capitalize ${update.persona === 'Alpha' ? 'text-purple-500 dark:text-purple-400' : 'text-cyan-500 dark:text-cyan-400'}`}>{getAgentInfo(update.persona, t).name}:</span>
                        ) : (
                             <span className="font-semibold capitalize text-gray-800 dark:text-gray-200">{t('thought')}:</span>
                        )}
